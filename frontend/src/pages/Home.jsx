...
const Home = () => {
    ...
    const fetchPosts = async (pageNum, isInitial = false) => {
        try {
            ...
            if (activeTab === 'saved') {
                ...
                newPosts = data;
            } else if (data && data.content) { // Standard Spring Page object
    ...
    const handlePostCreated = (newPost) => {
        ...
        sessionStorage.setItem(HOME_CACHE_KEY, JSON.stringify([newPost, ...userPosts]));
    };
    ...
    return (
        <>
            <main className="home-main">
                ...
                <div className="greeting-card" onClick={() => window.dispatchEvent(new CustomEvent('openCreatePost'))}>
                ...
                {initialLoading && userPosts.length === 0 ? (
                    ...
                    <div className="user-posts-section">
                        {activeTab === 'all' && <h2 className="section-title">Bài viết mới nhất</h2>}
                        {activeTab === 'following' && <h2 className="section-title">Từ những người bạn theo dõi</h2>}
                        {activeTab === 'saved' && <h2 className="section-title">Bài viết bạn đã lưu</h2>}
                ...
                {!initialLoading && userPosts.length === 0 && !error && (
                    <div style={{ padding: '40px 20px', textAlign: 'center', color: 'var(--text-secondary)', backgroundColor: 'var(--card-bg)', borderRadius: '12px' }}>
