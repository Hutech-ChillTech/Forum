  ...
  const handleFollowToggle = async () => {
    ...
    try {
      if (isFollowing) {
        ...
        setIsFollowing(result?.alreadyFollowing ? true : true);
      }
    } catch {
      // Re-fetch real state from server on error
      followService
        .getFollowStatus(userId)
        .then((s) => setIsFollowing(s?.isFollowing ?? false))
        .catch(() => {});
    } finally {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        ...
        if (targetUser && activeTab === "posts") {
          // Only update name/avatar if on main tab load
          setUserData((prev) => ({
            ...prev,
            name:
              targetUser.fullName ||
  ...
  const handleAvatarChange = async (e) => {
    ...
    try {
      ...
      window.dispatchEvent(
        new CustomEvent("userProfileUpdated", { detail: newProfile }),
      );
      ...
      <main className="profile-main">
        ...
        <div className="profile-info-section">
          ...
          <p className="profile-bio">
            {userData.bio ||
              "Thành viên đam mê công nghệ và chia sẻ kiến thức tại SkillForum."}
          </p>
