...
backend/
├── .mvn/wrapper/                  # Contains Maven Wrapper scripts
├── pom.xml                        # Maven project configuration and dependencies file
└── src/
    ├── main/
    │   ├── java/com/forum/it/     # CORE DIRECTORY (Contains all Java source code)
    │   │   ├── configs/           # System configuration files (Redis, CORS, Beans, etc.)
    │   │   ├── contants/          # Classes defining constant values (Constants).
    │   │   ├── controllers/       # API endpoints that receive requests from clients.
    │   │   ├── dtos/              # Data Transfer Objects – used to transfer data.
    │   │   │   ├── request/       # DTOs for receiving data from clients (e.g. LoginRequest).
    │   │   │   └── response/      # DTOs for formatting responses returned to clients (e.g. UserResponse).
    │   │   ├── entities/          # Models/Entities mapped directly to database tables.
    │   │   │   ├── post/          # Entities related to posts.
    │   │   │   ├── system/        # System-related entities.
    │   │   │   ├── tag/           # Entities related to tags.
    │   │   │   └── user/          # Entities related to users/accounts.
    │   │   ├── exceptions/        # Handling and processing exceptions (Global Exception Handler).
    │   │   ├── repositories/      # Interfaces for database interaction.
    │   │   │   └── interfaces/    # Repository interface definitions.
    │   │   ├── sercurites/        # Security configuration, authorization, JWT tokens (Authentication, Filters).
    │   │   └── services/          # Business logic layer (core application logic).
    │   │
    │   └── resources/             # RESOURCES DIRECTORY
    │       ├── application.yml/properties  # Configuration file for DB, server port, environment, etc.
    │       └── db/migration/               # SQL scripts (e.g. Flyway) for automatic DB schema creation/migration.
    │
    └── test/                      # TEST DIRECTORY
        ├── java/com/forum/it/     # Unit tests for application logic.
        └── resources/             # Test environment resources, similar to main resources.
